...
| `static/admin/` | Add (Directory) | Dashboard files, served by Auth Lambda from deployment package |
...
| Static file location | S3/CloudFlare Pages, Lambda inline, Lambda deployment package | static/admin/ served by Auth Lambda | No new infrastructure; files packaged in Lambda zip, served via /admin/* route |
