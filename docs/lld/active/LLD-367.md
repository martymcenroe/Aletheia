...
Update Reason: Revision #3 - Fixed requirement coverage gaps and added CLI safety guard per Gemini Review #1
-->
...
MAX_BATCH_SIZE: int = 1000  # Safety limit for code generation
...
def generate_coupons(tier: str, count: int, expires_days: int, max_uses: int = 1) -> list[str]:
    ...
    Raises:
        ValueError: If count exceeds MAX_BATCH_SIZE (1000)
    """
    ...
...
def redeem_coupon(code: str, user_id: str) -> dict:
    ...
    Logs all redemption attempts (success and failure) for audit.
...
3. Validate count <= MAX_BATCH_SIZE (1000)
   IF count > MAX_BATCH_SIZE THEN raise ValueError
...
5. Output generated codes to stdout

=== User Redeem Coupon ===
1. Receive POST /redeem-coupon with {code: string}
2. Normalize code to uppercase
3. Validate format matches ^[A-Z0-9]{16}$
   IF invalid THEN log attempt AND return {error: "invalid_code", message: "Invalid coupon code"}
4. Get coupon record from DynamoDB
   IF not found THEN log attempt AND return {error: "invalid_code", message: "Invalid coupon code"}
5. Check revoked flag
   IF revoked THEN log attempt AND return {error: "invalid_code", message: "Invalid coupon code"}
6. Check expiry
   IF now > expiry THEN log attempt AND return {error: "code_expired", message: "This code has expired"}
7. Check uses < max_uses
   IF uses >= max_uses THEN log attempt AND return {error: "code_exhausted", message: "This code has reached its usage limit"}
8. Atomic update with conditions:
   ...
   IF condition fails THEN log attempt AND return appropriate error (race condition lost)
9. Update user tier in aletheia-users table
10. Log successful redemption
11. Return {success: true, tier: coupon.tier}
...
* **Key Decisions:**
  ...
  - MAX_BATCH_SIZE safety limit prevents runaway CLI operations
  - All redemption attempts logged for audit/debugging
...
| CLI batch safety | No limit, config-based, hardcoded | Hardcoded MAX_BATCH_SIZE=1000 | Prevents accidental runaway; can be increased intentionally |
...
9. System collects and validates user email addresses (optional field)
10. Test fixtures provide mock data for automated testing
11. CLI enforces safety limit on batch code generation (MAX_BATCH_SIZE=1000)
12. All redemption attempts are logged for audit/debugging (CloudWatch)

**Note:** Privacy policy update requirements are tracked as a blocking gate in Definition of Done (Section 12) rather than a code requirement.
...
| Unbounded CLI loop | MAX_BATCH_SIZE=1000 hardcoded safety limit | Addressed |
...
| CloudWatch logs | $0.50/GB | ~1MB/month | < $0.01 |
...
- [x] MAX_BATCH_SIZE prevents runaway generation costs
...
| T170 | test_configurable_expiry | Expiry calculated correctly | RED |
| T180 | test_configurable_max_uses | max_uses stored correctly | RED |
| T190 | test_dynamodb_schema_documented | Documentation file exists | RED |
| T200 | test_fixtures_exist | Fixture file is valid JSON | RED |
| T210 | test_generate_exceeds_batch_limit | Raises ValueError when count > 1000 | RED |
| T220 | test_redemption_logs_attempts | CloudWatch receives log entry | RED |
...
| 210 | Batch generation limit (REQ-11) | Auto | generate --count 1001 | ValueError raised | Error message mentions MAX_BATCH_SIZE |
| 220 | Redemption logging (REQ-12) | Auto | Redeem any code | Log entry created | CloudWatch log contains user_id and code |
...
| Unbounded CLI loop | Medium | Low | MAX_BATCH_SIZE=1000 hardcoded in generate_coupons() |
...
### Tests
- [ ] All test scenarios pass (22 scenarios)
...
| MAX_BATCH_SIZE limit | `generate_coupons()` |
| Redemption logging | `redeem_coupon()` |
...
| Revision #2 | 2026-02-16 | REJECTED | Added tests for REQ-1,2,6,8,9,10 coverage |
| Gemini Review #1 | 2026-02-16 | REVISE | REQ-9 mismatch, missing CLI safety limit, missing audit logging |
| Revision #3 | 2026-02-16 | PENDING | Fixed all Gemini feedback |
...
### Gemini Review #1 Comments

| ID | Comment | Implemented? |
|----|---------|--------------|
| G1.1 | "REQ-9 maps to tests 140/150 which verify email validation, not privacy policy documentation" | YES - Changed REQ-9 to "System collects and validates user email addresses"; privacy policy moved to DoD blocking gate |
| G1.2 | "Unbounded Loop in CLI: --count argument needs safety limit" | YES - Added MAX_BATCH_SIZE=1000 constant, validation in generate_coupons(), test scenario 210 |
| G1.3 | "Missing Audit Logging: no explicit requirement/test for server-side logging of redemption attempts" | YES - Added REQ-12 for logging, added logging steps in pseudocode, test scenario 220 |
| G1.4 | "Tests 140/150 exist but map to non-existent requirement" | YES - REQ-9 now explicitly covers email collection and validation |
