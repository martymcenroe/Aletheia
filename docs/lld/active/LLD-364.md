...
Update Reason: Fixed mechanical test plan validation - added missing requirement coverage for REQ-3, REQ-4, REQ-7
-->
...
| T150 | test_tier_config_loaded_from_dynamodb | Tier config retrieved from DynamoDB | RED |
| T160 | test_admin_tier_caps_applied | Admin tier uses 50/500/10000 caps | RED |
...
|----|----------|------|-------|-----------------|---------------|
| 010 | Request under all limits (REQ-1) | Auto | user_id, free tier, counts=0 | allowed=True, counts=[1,1,1] | Counter state shows all incremented |
| 020 | Hourly limit exceeded (REQ-1) | Auto | free tier, hourly_count=5 | allowed=False, window="hourly" | 429 with hourly window info |
| 030 | Daily limit exceeded (REQ-1) | Auto | free tier, daily_count=15 | allowed=False, window="daily" | 429 with daily window info |
| 040 | Monthly limit exceeded (REQ-1) | Auto | free tier, monthly_count=100 | allowed=False, window="monthly" | 429 with monthly window info |
| 050 | Free tier boundary hourly (REQ-3) | Auto | 5 requests same hour | First 5 succeed, 6th fails | Counter=5, then 429 |
| 060 | Subscriber tier boundary hourly (REQ-3) | Auto | 20 requests same hour | First 20 succeed, 21st fails | Counter=20, then 429 |
| 070 | JWT tier claim present (REQ-4) | Auto | User with tier=subscriber | JWT contains tier="subscriber" | Decode JWT, verify claim |
| 080 | DynamoDB timeout fail-open (REQ-6) | Auto | Simulate 2s+ timeout | allowed=True, metric incremented | Request proceeds, metric logged |
| 090 | Atomic transaction failure (REQ-2) | Auto | Simulate mid-transaction failure | No counters incremented | All counts remain at previous values |
| 100 | Monthly anniversary reset (REQ-8) | Auto | anchor=16, date=17th | New monthly window started | Monthly count=1 (not accumulated) |
| 110 | Counter TTL verification (REQ-7) | Auto | Create counter items | TTL attributes set correctly | hourly=7200, daily=172800, monthly=3024000 |
| 120 | 429 response format (REQ-5) | Auto | Rate limit exceeded | JSON with all required fields | Validate against schema |
| 130 | Tier config caching (REQ-3) | Auto | Two calls within 5 min | DynamoDB called once | Mock verifies single call |
| 140 | Missing tier defaults to free (REQ-4) | Auto | JWT without tier claim | Free tier limits applied | 6th hourly request returns 429 |
| 150 | Static fixture validity (REQ-5) | Auto | Load fixture JSON | Valid 429 response structure | JSON schema validation |
| 160 | Multiple windows exceeded (REQ-1) | Auto | All three at limit | Returns first exceeded (hourly) | Window order: hourly > daily > monthly |
| 170 | Tier config loaded from DynamoDB (REQ-3) | Auto | Tier config in DynamoDB | Config retrieved and applied | Caps match stored values |
| 180 | Admin tier caps applied (REQ-3) | Auto | Admin user requests | 50/500/10000 caps used | 51st hourly request returns 429 |
| 190 | Counter TTL hourly window (REQ-7) | Auto | Create hourly counter | TTL set to 2 hours | Item has TTL=7200s from now |
| 200 | Counter TTL daily window (REQ-7) | Auto | Create daily counter | TTL set to 2 days | Item has TTL=172800s from now |
| 210 | Counter TTL monthly window (REQ-7) | Auto | Create monthly counter | TTL set to 35 days | Item has TTL=3024000s from now |
| 220 | Tier embedded at issuance no lookup (REQ-4) | Auto | User authenticates | No per-request user table read | Mock verifies zero GetItem calls during request |
...
| Mechanical Validation | 2026-02-16 | REJECTED | Missing test coverage for REQ-3, REQ-4, REQ-7 |
