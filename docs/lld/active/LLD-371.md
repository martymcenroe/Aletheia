...
Update Reason: Fixed mechanical validation errors - corrected test scenario coverage to map all requirements
-->
...
|---------|------------------|-------------------|--------|
| T010 | Product copy file exists with value prop, features, pricing | File present with required sections | RED |
| T020 | LinkedIn post draft exists | Markdown file present in docs/lineage/active/371-lld/ | RED |
| T030 | HTML snippets contain auth/rate limiting features | Auth and rate limiting mentioned in HTML | RED |
| T040 | Screenshots present (3 PNG files) | 3 PNG files in docs/lineage/active/371-lld/ | RED |
| T050 | Pricing section exists in product copy | Free/subscriber tiers documented | RED |
| T060 | Chrome badge markup present in HTML | Badge/link element in HTML snippets | RED |
| T070 | Implementation report documents deployment | Report exists with deployment steps and verification | RED |
...
|----|----------|------|-------|-----------------|---------------|
| 010 | Product copy file exists with required content (REQ-1) | Auto | File system check + content parse | File present with value prop, features, pricing | aletheia-product-copy.md contains "value", "feature", "pricing" sections |
| 020 | LinkedIn post draft exists (REQ-2) | Auto | File system check | File present | aletheia-launch-linkedin.md exists in docs/lineage/active/371-lld/ |
| 030 | HTML snippets contain auth/rate limiting features (REQ-3) | Auto | grep HTML snippets | Match found | "auth" and "rate limit" appear in aletheia-study-updates.html |
| 040 | Screenshots present (REQ-4) | Auto | File system check | 3 files | 3 PNG files in docs/lineage/active/371-lld/ |
| 050 | Pricing section exists in product copy (REQ-5) | Auto | Parse product copy | Section found | "free" and "subscriber" text present in aletheia-product-copy.md |
| 060 | Chrome badge markup present in HTML (REQ-6) | Auto | Parse HTML snippets | Element found | Chrome Store link/badge in aletheia-study-updates.html |
| 070 | Implementation report documents deployment (REQ-7) | Auto | File system + content check | Report exists with required sections | implementation-report.md contains deployment steps and verification sections |
| 080 | No PII in screenshots | Manual | Visual inspection | No personal data | No real names, emails, photos visible |
| 090 | Message consistency across drafts | Manual | Cross-document review | Matching content | Same value prop in all draft files |
...
|----|----------|-------------------|-------|
| 080 | No PII in screenshots | Requires visual interpretation of image content | 1. Open each screenshot 2. Verify no real names/emails/photos 3. Document in report |
| 090 | Message consistency | Requires semantic comparison of marketing copy | 1. Review all draft files 2. Compare value propositions 3. Note any discrepancies |
...
| Revision #1 | 2026-02-16 | REJECTED | Fixed paths, but test coverage incomplete (28.6%) |
| Revision #2 | 2026-02-16 | PENDING | Added REQ-N suffixes to all test scenarios |
