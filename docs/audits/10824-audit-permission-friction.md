...
## 9. Zugzwang Protocol (Real-Time Friction Escalation)

**Purpose:** Process real-time permission friction events logged to `AgentOS/logs/zugzwang.log` and escalate to GitHub issue tracking.

**GitHub Issue:** https://github.com/anthropics/claude-code/issues/17637

### 9.1 Zugzwang Log Format

Location: `/c/Users/mcwiz/Projects/AgentOS/logs/zugzwang.log`

Format: `YYYY-MM-DDTHH:MM:SS | <event details>`

**Event Types:**
- Permission prompts: `Do you want to create/edit/overwrite X?`
- Tilde path generation: `Read(~\Projects\...)`, `Search(path: "~\...")`
- Bash command prompts: `mkdir -p ...`, `cp ...`
- Gemini invocation issues: `gemini -p "..."`

### 9.2 Processing Procedure

**When user says "check zugzwang" or "process friction log":**

1. **Read the log**
2. **For each unique pattern:**
   - If not already documented → Comment on GitHub issue #17637
   - Include: exact prompt, expected matching pattern, tool type
3. **Clear the log**
4. **Report summary to user**

### 9.3 Known Broken Patterns (2026-01-12)

**All permission patterns are non-functional on Windows:**

| Tool | Pattern Configured | Works? |
|------|-------------------|--------|
| Read | `Read(**)` | NO |
| Write | `Write(C:\\Users\\mcwiz\\Projects\\**)` | NO |
| Edit | `Edit(C:\\Users\\mcwiz\\Projects\\**)` | NO |
| Glob | `Glob(**)` | NO |
| Search | `Search(**)` | NO |
| Bash | `Bash(mkdir:*)`, `Bash(gemini:*)`, etc. | NO |

**Root Cause:** Claude Code permission system bug

**Workaround:** Manual approval or Shift+Tab for session-wide directory approval

### 9.4 Escalation Philosophy

> "Blast them with comments. 10,000 comments if needed."

Every friction event gets documented. Volume demonstrates severity.
...
- AgentOS/logs/zugzwang.log - Real-time friction log
- https://github.com/anthropics/claude-code/issues/17637 - Permission bug tracking
