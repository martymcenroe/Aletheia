...
- After major architectural changes (e.g., LangGraph → Naked Python)
...
```bash
# Example: Issue #45 (Denylist)
# Check if the feature exists
ls src/guardrails/denylist.py  # If exists, issue may be complete
grep -r "check_denylist" src/  # If function exists, verify
```
...
- Old architecture (LangGraph, LangChain, LangSmith)
- Old terminology (L1/L2/L3/L4 instead of Selection/Denylist/Semantic/Transform)
- Superseded approaches (RSDB instead of Wikipedia)
...
| Issue | Reason |
|-------|--------|
| #45 | Code exists in src/guardrails/denylist.py |

#### Closed (Deprecated)
| Issue | Superseded By |
|-------|---------------|
| #119 | #121 (Wikipedia replaces RSDB) |

#### Updated (Terminology)
| Issue | Changes |
|-------|---------|
| #126 | Added "Wikipedia-sourced" to denylist reference |

#### Flagged (Stale)
| Issue | Last Updated | Action |
|-------|--------------|--------|
| #84 | 2025-12-24 | Added post-mvp label |
```

## Integration

- Run before using `docs/6000-open-issues.md` for planning
- Results inform IMMEDIATE-PLAN.md updates
