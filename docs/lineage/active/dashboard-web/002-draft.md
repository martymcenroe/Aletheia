...
- [x] **Cost:** DynamoDB scans for metrics (mitigated by 5-minute caching). CloudWatch GetMetricData calls.
...
## Dependencies
- Issue #TBD (dashboard-cloudwatch) must be completed first — need CloudWatch metrics emitting for operational metrics portion

> **Note:** The dependency issue for CloudWatch metrics emitting should be created before filing this issue. Update `#TBD` with actual issue number.
...
- [ ] Dashboard displays error message when API returns 5xx
- [ ] Dashboard auto-refreshes data every 5 minutes (visible timestamp updates)
- [ ] Dashboard renders correctly on viewport width 375px (mobile)
- [ ] Response time for `/metrics` endpoint < 3 seconds (cold start acceptable)
