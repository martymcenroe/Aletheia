...
Update Reason: Initial LLD creation for Issue #367
-->
...
| `lambda/auth/coupon_handler.py` | Add | Redemption endpoint logic for POST /redeem-coupon |
| `lambda/auth/serverless.yml` | Modify | Add /redeem-coupon route configuration |
| `terraform/dynamodb.tf` | Modify | Add aletheia-coupons table definition |
| `extension/src/components/Profile/CouponRedemption.tsx` | Add | React component for coupon redemption UI |
| `extension/src/components/Profile/EmailInput.tsx` | Add | React component for email collection |
| `extension/src/api/coupon.ts` | Add | API client for coupon redemption |
| `extension/src/api/__mocks__/coupon.ts` | Add | Mock API client with static fixtures |
| `docs/privacy-policy.md` | Modify | Update with email collection disclosure |
...
```json
// extension/package.json - no new dependencies required
// React and existing API infrastructure sufficient
```
...
// extension/src/components/Profile/CouponRedemption.tsx

interface CouponRedemptionProps {
  onSuccess: (tier: string) => void;
}

export function CouponRedemption(props: CouponRedemptionProps): JSX.Element;

// extension/src/components/Profile/EmailInput.tsx
...
* **Module:** `tools/admin_coupons.py`, `lambda/auth/coupon_handler.py`, `extension/src/components/Profile/`
* **Pattern:** Command pattern for CLI, atomic conditional writes for DynamoDB, controlled forms for React
* **Key Decisions:**
  ...
  - Mock API client enables offline UI development
...
4. Users can redeem valid codes via extension popup
...
8. Users can optionally provide email address in profile section
9. Email is validated client-side before submission
10. Privacy policy is updated before feature ships (blocking gate)
...
|---------|--------|-------|
| Valid coupon response | Hardcoded | `{"success": true, "tier": "subscriber"}` |
| Expired code response | Hardcoded | `{"error": "code_expired", "message": "This code has expired"}` |
| Exhausted code response | Hardcoded | `{"error": "code_exhausted", "message": "This code has reached its usage limit"}` |
| Invalid code response | Hardcoded | `{"error": "invalid_code", "message": "Invalid coupon code"}` |
| Test coupon records | Generated | Created in test setup, deleted in teardown |
...
Local Dev ──commit──► GitHub ──merge──► Main Branch
                                            │
                                      ──terraform apply──► DynamoDB table created
                                            │
                                      ──serverless deploy──► Lambda updated
                                            │
                                      ──npm build──► Extension published
```
...
| Extension load | < 100ms added | Lazy load coupon component |
...
| 030 | Generate stores in DynamoDB | Auto | generate --tier sub --count 5 | 5 records | scan returns 5 items with tier="sub" |
...
| 090 | Redemption updates user tier | Auto | Redeem valid code | User record has new tier | GetItem shows tier=subscriber |
...
cd extension && npm test -- --testPathPattern="Coupon|Email"
```
...
Extension UI testing automated via React Testing Library. No visual inspection required for MVP.
...
- [ ] **BLOCKING:** Privacy policy updated with email collection disclosure
