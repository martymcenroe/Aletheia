...
Update Reason: Initial LLD creation for multi-window tiered rate limiting
-->
...
|------|-------------|-------------|
| `aletheia/services/token_cap_service.py` | Modify | Add `MultiWindowCounter` class, tier config loading, transactional increment |
| `aletheia/middleware/auth_middleware.py` | Modify | Extract tier from JWT, pass to rate limit check, handle 429 responses |
| `aletheia/lambda/lambda_auth_function.py` | Modify | Embed `tier` claim in JWT at issuance |
| `aletheia/models/user.py` | Modify | Add `tier` and `billing_anchor_day` fields |
| `aletheia/models/rate_limit.py` | Add | New models for tier config, counter state, and 429 response |
| `aletheia/services/tier_config_service.py` | Add | Service for loading/caching tier configurations from DynamoDB |
| `tools/admin_token_cap.py` | Modify | Add `set-tier`, `get-tier`, `config-tier` subcommands |
| `extension/src/components/RateLimitModal.tsx` | Add | Modal component for upgrade nudge UX |
| `extension/src/api/client.ts` | Modify | Handle 429 responses, trigger modal |
| `extension/src/hooks/useRateLimit.ts` | Add | Hook for managing rate limit state and countdown |
...
| `tools/README.md` | Modify | Document new CLI subcommands |
...
// Uses existing React, TypeScript dependencies
```
...
```python
# aletheia/services/token_cap_service.py
...
# aletheia/services/tier_config_service.py
...
# aletheia/middleware/auth_middleware.py (additions)
...
# aletheia/lambda/lambda_auth_function.py (additions)
...
# tools/admin_token_cap.py (additions)

def cmd_set_tier(user_id: str, tier: str) -> None:
    """Set tier for specified user in aletheia-users table."""
    ...

def cmd_get_tier(user_id: str) -> None:
    """Display current tier for specified user."""
    ...
...
def cmd_config_tier(
    ...
    """Update tier configuration caps."""
    ...

def cmd_show_config(tier: Optional[str] = None) -> None:
    """Display tier configuration(s)."""
    ...
```
...
**CLI Tier Management:**
```
1. Parse command: set-tier | get-tier | config-tier | show-config
2. Validate inputs:
   - tier must be free|subscriber|admin
   - caps must be positive integers
3. Execute DynamoDB operation
4. Print result/confirmation
```

### 2.6 Technical Approach

* **Module:** `aletheia/services/token_cap_service.py`, `aletheia/services/tier_config_service.py`
...
7. Admin CLI can set user tiers and update tier configurations
8. Counter items have appropriate TTLs (hourly=2h, daily=2d, monthly=35d)
9. Monthly window respects user's billing_anchor_day for anniversary-based reset
10. Extension displays modal with reset countdown on 429 response
...
|---------|--------|-------|
| `tests/fixtures/rate_limit_429_response.json` | Hardcoded | Standard 429 response body for frontend testing |
...
- **Production:** Production DynamoDB tables; tier configs pre-seeded via CLI

**External utility needed:** No separate utility; CLI tool handles config seeding.
...
| Admin CLI unauthorized access | Requires valid AWS credentials with DynamoDB write permissions | Addressed |
...
| Counter corruption | TTL auto-cleanup; admin can reset via CLI | Addressed |
...
2. Admin CLI can manually reset counters if needed
...
| T120 | test_cli_set_tier_updates_user | User record updated in DynamoDB | RED |
| T130 | test_cli_config_tier_updates_config | Tier config updated in DynamoDB | RED |
| T140 | test_429_response_includes_reset_time | Response has resets_at and resets_in_seconds | RED |
| T150 | test_tier_config_cache_hit | Second call uses cached value | RED |
| T160 | test_missing_tier_claim_defaults_to_free | No tier in JWT → free limits applied | RED |
...
| 120 | CLI set-tier command | Auto | set-tier --user-id X --tier admin | User record updated | Get user shows tier=admin |
| 130 | CLI config-tier command | Auto | config-tier --tier free --hourly 10 | Config updated | Free tier hourly_cap=10 |
| 140 | 429 response format | Auto | Rate limit exceeded | JSON with all required fields | Validate against schema |
| 150 | Tier config caching | Auto | Two calls within 5 min | DynamoDB called once | Mock verifies single call |
| 160 | Missing tier defaults to free | Auto | JWT without tier claim | Free tier limits applied | 6th hourly request returns 429 |
| 170 | Static fixture validity | Auto | Load fixture JSON | Valid 429 response structure | JSON schema validation |
| 180 | Multiple windows exceeded | Auto | All three at limit | Returns first exceeded (hourly) | Window order: hourly > daily > monthly |
...
# Run with coverage
poetry run pytest tests/unit/test_multi_window_counter.py --cov=aletheia/services --cov-report=html
```
...
|----|----------|-------------------|-------|
| M01 | Extension modal UX | Visual inspection of countdown animation | 1. Trigger 429 in browser dev tools, 2. Verify modal appears, 3. Verify countdown updates, 4. Click Dismiss, 5. Trigger another 429, 6. Verify modal reappears |
| M02 | Upgrade button navigation | Requires real subscription flow | 1. Click Upgrade in modal, 2. Verify navigation to subscription page |
...
| Tier config cache staleness | Users get wrong limits for up to 5 min | Low | Acceptable tradeoff; admin can invalidate cache |
...
### Tests
- [ ] All test scenarios pass (T010-T180)
...
- [ ] `tools/README.md` updated with CLI documentation
...
- `aletheia/services/token_cap_service.py` ✓ (Section 2.1)
- `aletheia/services/tier_config_service.py` ✓ (Section 2.1)
- `aletheia/middleware/auth_middleware.py` ✓ (Section 2.1)
- `aletheia/lambda/lambda_auth_function.py` ✓ (Section 2.1)
- `tools/admin_token_cap.py` ✓ (Section 2.1)
- `tools/README.md` ✓ (Section 2.1)
