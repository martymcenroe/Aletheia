...
|------|-------------|-------------|
| `src/auth/` | Add (Directory) | Directory for auth-related modules including Stripe integration |
| `src/auth/stripe_handler.py` | Add | Webhook handler, checkout session creation, signature validation |
| `src/auth/stripe_events.py` | Add | Event type handlers for tier changes and grace period logic |
| `src/lambda_function.py` | Modify | Add routing for `/create-checkout-session`, `/stripe-webhook`, `/subscription-status` |
| `extension/popup/` | Add (Directory) | Directory for extension popup components |
| `extension/popup/subscription-status.js` | Add | Subscription UI component with tier display and upgrade button |
| `extension/popup/popup.html` | Add | Popup HTML with subscription status section container |
| `extension/popup/popup.css` | Add | Styles for subscription UI elements |
| `tools/admin_subscriptions.py` | Add | CLI tool for subscription management with --dry-run mode |
| `docs/adrs/0015-stripe-integration.md` | Add | Architecture decision record for Stripe integration |
...
- Changed `auth-lambda/` paths to `src/auth/` (using existing `src/` directory)
- Changed `auth-lambda/lambda_function.py` to `src/lambda_function.py` (existing file)
- Changed `docs/adr/` to `docs/adrs/` (existing directory)
- Added `extension/popup/` as new directory to be created
- Removed `infrastructure/secrets.tf` and `infrastructure/api-gateway.tf` (infrastructure directory doesn't exist - infrastructure changes handled via existing deployment process)
- Removed `docs/0003-file-inventory.md` modification (file doesn't exist)
...
* **Module:** `src/auth/` (new submodule for Stripe integration)
...
- Must integrate with existing Lambda routing pattern in `src/lambda_function.py`
...
| T140 | test_replay_attack_prevention | Same webhook resent is rejected | RED |
| T150 | test_secrets_retrieval | API keys retrieved from Secrets Manager | RED |
| T160 | test_extension_upgrade_button | Upgrade button initiates checkout flow | RED |
...
|----|----------|------|-------|-----------------|---------------|
| 010 | Checkout session creation (REQ-1) | Auto | User ID, email | Checkout URL with session ID | URL starts with https://checkout.stripe.com |
| 020 | Valid webhook signature (REQ-8) | Auto | Signed payload from fixture | Event processed | Returns 200, tier updated |
| 030 | Invalid webhook signature (REQ-8) | Auto | Tampered payload | Rejection | Returns 400, no tier change |
| 040 | checkout.session.completed (REQ-3) | Auto | Event fixture | Tier upgrade | User tier = "premium" in DB |
| 050 | invoice.paid during grace (REQ-4) | Auto | Event fixture, user in grace | Grace cleared | grace_period_end = None |
| 060 | invoice.payment_failed (REQ-4) | Auto | Event fixture | Grace initiated | grace_period_end = now + 7 days |
| 070 | subscription.deleted (REQ-5) | Auto | Event fixture | Tier downgrade | User tier = "free" in DB |
| 080 | Duplicate event (REQ-6) | Auto | Same event ID twice | Idempotent | Second call returns 200, no DB change |
| 090 | Status - free user (REQ-10) | Auto | Free tier user | Status response | {tier: "free", status: "none"} |
| 100 | Status - premium user (REQ-10) | Auto | Premium tier user | Status response | {tier: "premium", status: "active"} |
| 110 | Status - grace period (REQ-10) | Auto | User with grace_period_end | Status response | {status: "grace_period", days: N} |
| 120 | Admin view subscription (REQ-9) | Auto | Valid user email | Subscription details | Returns tier, stripe IDs, status |
| 130 | Admin dry-run adjustment (REQ-9) | Auto | Email, new tier, --dry-run | Planned changes | Outputs changes, DB unchanged |
| 140 | Replay attack prevention (REQ-6) | Auto | Same webhook payload resent | Rejection | Event ID tracked, no duplicate processing |
| 150 | Secrets from Secrets Manager (REQ-7) | Auto | Lambda cold start | API keys loaded | Keys retrieved from Secrets Manager, not hardcoded |
| 160 | Checkout with pre-filled email (REQ-2) | Auto | User with email | Checkout session | Session contains customer_email field |
...
- [ ] ADR created at `docs/adrs/0015-stripe-integration.md`
...
- [ ] 0809 Security Audit - PASS (payment handling is security-critical)
- [ ] 0810 Privacy Audit - PASS (handles customer IDs linked to users)
...
- `tools/admin_subscriptions.py` ✓
- `docs/adrs/0015-stripe-integration.md` ✓
...
| Mechanical #1 | 2026-02-16 | REJECTED | REQ-1 and REQ-7 missing test coverage |
...
|----|---------|--------------|
| M1.1 | "Requirement REQ-1 has no test coverage" | YES - Added scenario 010 with (REQ-1) and scenario 160 with (REQ-2) |
| M1.2 | "Requirement REQ-7 has no test coverage" | YES - Added scenario 150 with (REQ-7) |
