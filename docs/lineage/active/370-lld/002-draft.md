...
Updated By: Issue #370 LLD creation
Update Reason: Initial LLD for documentation catchup work
-->
...
|------|-------------|-------------|
| `docs/adr/10217-jwt-authentication.md` | Add | JWT authentication architecture decision record |
| `docs/adr/10218-daily-token-cap.md` | Add | Daily token cap implementation decision record |
| `docs/adr/10219-auth-middleware-pattern.md` | Add | Auth middleware pattern decision record |
| `docs/adr/10200-index.md` | Modify | Update ADR index with new entries |
| `docs/retrospectives/2026-02-pre-launch.md` | Add | Lessons learned compilation from recent sessions |
| `docs/0003-file-inventory.md` | Modify | Add new documentation files to inventory |

**Note:** Blog drafts are created in the separate `dispatch` repository and are not tracked in AssemblyZero's file inventory.
...
**Validation Notes:**
- `docs/adr/` exists (contains existing ADRs)
- `docs/retrospectives/` may need to be created if it doesn't exist
- `docs/0003-file-inventory.md` exists
...
4. Update ADR Index (10200)
   a. Add entries for 10217, 10218, 10219
   b. Maintain alphabetical/numerical ordering

5. Create Lessons Learned Document
   a. Categorize by tooling, environment, packaging
   b. Cross-reference issue numbers (#380, #382)
   c. Include specific commands and fixes

6. Create Blog Drafts (dispatch repo)
   a. Privacy-First Auth outline
   b. CloudFlare Workers Gateway outline
   c. Adversarial Auditing outline

7. Update File Inventory
   a. Add all new docs/adr/*.md files
   b. Add docs/retrospectives/*.md file
```
...
| Lessons Learned Location | `docs/`, `docs/retrospectives/`, wiki | `docs/retrospectives/` | Keeps operational knowledge in version control alongside code |
| Blog Draft Location | AssemblyZero repo, dispatch repo | dispatch repo | Follows established blog publishing workflow |
...
- Blog drafts must follow dispatch repo frontmatter format
...
6. At least one blog draft exists in dispatch repo with title, intro, and section outline
7. All ADRs pass markdown linting (no syntax errors)
8. File inventory updated with all new documentation files
...
**Rationale:** Keeping documentation in the repository ensures it stays synchronized with code changes and is subject to the same review process.
...
| T070 | All markdown files pass linting | No syntax errors from ruff/markdownlint | RED |
...
|----|----------|------|-------|-----------------|---------------|
| 010 | ADR 10217 structure | Auto | `docs/adr/10217-jwt-authentication.md` | Valid markdown with all sections | grep finds Status, Context, Decision, Consequences |
| 020 | ADR 10218 structure | Auto | `docs/adr/10218-daily-token-cap.md` | Valid markdown with all sections | grep finds Status, Context, Decision, Consequences |
| 030 | ADR 10219 structure | Auto | `docs/adr/10219-auth-middleware-pattern.md` | Valid markdown with all sections | grep finds Status, Context, Decision, Consequences |
| 040 | ADR index completeness | Auto | `docs/adr/10200-index.md` | Contains all three new ADR entries | grep finds 10217, 10218, 10219 |
| 050 | Lessons learned content | Auto | `docs/retrospectives/2026-02-pre-launch.md` | At least 6 categorized items | Line count of list items ≥ 6 |
| 060 | Blog draft structure | Auto | dispatch repo draft | Title, intro, sections present | grep finds # heading and ## subheadings |
| 070 | Markdown lint pass | Auto | All new .md files | No lint errors | `markdownlint` or `ruff` exit 0 |

### 10.2 Test Commands

```bash
# Verify ADR sections exist
grep -E "^## (Status|Context|Decision|Consequences)" docs/adr/10217-jwt-authentication.md
grep -E "^## (Status|Context|Decision|Consequences)" docs/adr/10218-daily-token-cap.md
grep -E "^## (Status|Context|Decision|Consequences)" docs/adr/10219-auth-middleware-pattern.md

# Verify index includes new ADRs
grep -E "1021[789]" docs/adr/10200-index.md

# Verify lessons learned has sufficient content
grep -c "^- " docs/retrospectives/2026-02-pre-launch.md  # Should be >= 6

# Markdown lint (if markdownlint installed)
markdownlint docs/adr/10217-jwt-authentication.md docs/adr/10218-daily-token-cap.md docs/adr/10219-auth-middleware-pattern.md
```
...
| dispatch repo not accessible | Low | Low | Blog drafts are non-blocking; ADRs are primary deliverable |
...
- [ ] File inventory updated with new files
...
|-------------|------------------|----------|
| ADR 10217 | `docs/adr/10217-jwt-authentication.md` | ✓ |
| ADR 10218 | `docs/adr/10218-daily-token-cap.md` | ✓ |
| ADR 10219 | `docs/adr/10219-auth-middleware-pattern.md` | ✓ |
| Index update | `docs/adr/10200-index.md` | ✓ |
| Lessons learned | `docs/retrospectives/2026-02-pre-launch.md` | ✓ |
