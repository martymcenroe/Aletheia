...
Update Reason: Fixed mechanical validation errors - added missing test coverage for REQ-2, REQ-6, REQ-9 through REQ-13
-->
...
| T170 | test_cta_opens_linkedin_page | CTA opens correct LinkedIn URL in new tab | RED |
| T180 | test_coupon_reward_amount | Coupon grants exactly +50 bonus requests | RED |
...
|----|----------|------|-------|-----------------|---------------|
| 010 | Successful follow verification (REQ-4, REQ-5) | Auto | Valid follower user_id | `{verified: true, coupon_code: "FOLLOW-...", reward: {type: "bonus_requests", amount: 50}}` | Response matches schema, coupon in DB |
| 020 | CTA opens LinkedIn page (REQ-2) | Auto | Click CTA button | New tab opened with `https://www.linkedin.com/company/thrivetech-ai` | URL matches expected, target="_blank" |
| 030 | Already claimed coupon (REQ-9) | Auto | User with existing coupon | `{verified: false, reason: "already_claimed", coupon_code: "FOLLOW-..."}` | Original coupon returned, no duplicate |
| 040 | Rate limit exceeded (REQ-10) | Auto | 4th attempt in 1 hour | HTTP 429 with Retry-After header | Status code 429, header present |
| 050 | LinkedIn API error | Auto | Mocked API 500 | `{verified: false, reason: "api_error"}` | Graceful error handling |
| 060 | Coupon grants +50 requests (REQ-6) | Auto | Redeem coupon | User balance increased by exactly 50 | Balance delta equals 50 |
...
| 090 | Idempotency - same coupon (REQ-9) | Auto | Two calls, same user | Identical coupon_code both times | Codes match |
...
| 160 | Token not persisted (REQ-11) | Auto | Complete OAuth flow | Query DB for tokens | No token records found |
...
| 190 | LinkedIn mapping stored (REQ-12) | Auto | Successful OAuth linking | Query DB for mapping | Record exists with user_id and linkedin_id |
| 200 | Data residency US-East-1 (REQ-13) | Auto | DynamoDB table config | Table region check | Region equals us-east-1 |
...
|----|----------|-------------------|-------|
| M010 | End-to-end follow flow (REQ-1, REQ-3, REQ-7, REQ-8) | Requires real LinkedIn account action and browser interaction | 1. Open extension popup 2. Verify CTA displays for free-tier user 3. Click CTA 4. Actually follow on LinkedIn 5. Click "I followed" 6. If not linked, complete OAuth popup 7. Verify coupon displayed with copy button 8. Check email received within 60 seconds |
| M020 | OAuth popup behavior (REQ-3) | Browser popup interaction | 1. Use account without LinkedIn linked 2. Click "I followed" 3. Verify OAuth popup opens 4. Complete OAuth 5. Verify verification continues |
...
|--------|------|---------|-----------|
| Mechanical #1 | 2026-02-16 | REJECTED | Missing test coverage for REQ-2, REQ-6, REQ-9, REQ-10, REQ-11, REQ-12, REQ-13 |
| - | - | - | Awaiting review |
