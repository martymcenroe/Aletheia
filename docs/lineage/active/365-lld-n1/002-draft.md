...
Updated By: Issue #365 LLD creation
Update Reason: Initial LLD for LinkedIn follower verification and coupon generation feature
-->
...
|------|-------------|-------------|
| `lambda/auth/verify_follow.py` | Add | New Lambda handler for follow verification endpoint |
...
| `extension/src/components/FollowIncentive.tsx` | Add | React component for follow CTA and verification UI |
| `extension/src/api/follow.ts` | Add | TypeScript API client for verify-follow endpoint |
| `database/migrations/002_add_linkedin_coupons.sql` | Add | Schema changes for coupon source tracking and LinkedIn ID mapping |
...
| `tools/README.md` | Modify | Document new admin tool |
...
async function verifyFollow(platform: string): Promise<VerifyFollowResponse>
async function initiateLinkedInOAuth(): Promise<string>  // Returns auth URL
async function completeLinkedInOAuth(code: string): Promise<boolean>
```
...
* **Module:** `lambda/auth/` (backend), `extension/src/components/` (frontend)
...
|---------|--------|-------|
| `linkedin_org_followers_response.json` | Generated | Mock follower list with test LinkedIn URNs |
| `linkedin_oauth_token_response.json` | Generated | Mock OAuth token response (ephemeral) |
| `linkedin_api_error_response.json` | Generated | Mock 429, 500, timeout responses |
| `coupon_dynamodb_item.json` | Generated | Sample DynamoDB coupon record |
...
# Run with coverage
poetry run pytest tests/unit/test_verify_follow.py tests/unit/test_linkedin_client.py --cov=lambda/auth --cov-report=term-missing
...
- [ ] `tools/README.md` updated with admin tool
