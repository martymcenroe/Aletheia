...
Update Reason: Fixed mechanical validation errors - corrected Section 3 format and Section 10.1 requirement references
-->
...
| `tests/fixtures/linkedin_org_followers_response.json` | Add | Mock response for follower verification |
...
|----|----------|------|-------|-----------------|---------------|
| 010 | Successful follow verification (REQ-1) | Auto | Valid user, LinkedIn follower | `{ verified: true, coupon_code: "FOLLOW-XXXXXXXX" }` | Response matches schema; coupon in DB |
| 020 | Coupon code generation format (REQ-2) | Auto | Verified follower | Coupon code FOLLOW-{8_ALPHANUMERIC} | Code matches pattern; +50 bonus requests |
| 030 | Already claimed coupon returns existing (REQ-3) | Auto | User with existing coupon | `{ verified: true, coupon_code: existing }` | Same coupon returned; no duplicate |
| 040 | Rate limit exceeded (REQ-4) | Auto | 4th request in 1 hour | HTTP 429 with Retry-After header | Rate limit enforced at 3/hour |
| 050 | LinkedIn API timeout (REQ-5) | Auto | Mocked timeout response | `{ verified: false, reason: "api_error" }` | Graceful degradation message |
| 060 | LinkedIn API 500 error (REQ-5) | Auto | Mocked 500 response | `{ verified: false, reason: "api_error" }` | Error logged; graceful response |
| 070 | Email sent within 60 seconds (REQ-6) | Auto | Verified follower | Email dispatched | Email sent < 60s after verification |
| 080 | OAuth token not persisted (REQ-7) | Auto | Complete OAuth flow | No token in DB after verification | Security requirement verified |
| 090 | Admin CLI verification (REQ-8) | Auto | Admin runs CLI tool | Coupon details returned | CLI outputs coupon info |
| 100 | User not following (REQ-1) | Auto | Valid user, not a follower | `{ verified: false, reason: "not_following" }` | No coupon created |
| 110 | Invalid user_id format | Auto | `user_id: "not-a-uuid"` | HTTP 400 Bad Request | Input validation |
| 120 | Invalid platform | Auto | `platform: "twitter"` | HTTP 400 Bad Request | Allowlist validation |
| 130 | Email send failure graceful (REQ-6) | Auto | Mock SES failure | Coupon still returned in response | Fire-and-forget behavior |
| 140 | Coupon code uniqueness | Auto | Generate 1000 codes | All codes unique | No collisions |
| 150 | Concurrent duplicate request (REQ-3) | Auto | Two simultaneous requests | Only one coupon created | Atomic idempotency |
...
| Mechanical Validation #2 | 2026-02-16 | REJECTED | Coverage: 37.5% (3/8 requirements mapped) |
