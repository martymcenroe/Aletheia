...
Updated By: Issue #369 LLD creation
Update Reason: Initial LLD for CloudWatch observability dashboard
-->
...
|------|-------------|-------------|
| `src/auth/middleware.py` | Modify | Add EMF structured logging for RequestCount, CapUtilization, CapDenied metrics |
| `src/auth/anonymize.py` | Add | New module with user ID hashing function |
| `src/handlers/response.py` | Modify | Add EMF structured logging for Latency, ErrorRate, BedrockCostEstimate metrics |
| `infrastructure/cloudwatch-dashboard.json` | Add | Dashboard widget definitions for Aletheia-Usage |
| `infrastructure/provision.sh` | Modify | Add dashboard creation, SNS topic, and alarm provisioning commands |
| `infrastructure/sns-alarm.json` | Add | SNS topic and CloudWatch alarm configuration |
| `infrastructure/contributor-insights-rules/top-talkers.json` | Add | Contributor Insights rule for abuse detection |
| `infrastructure/logs-insights-queries/active-users.sql` | Add | Logs Insights query for unique user count |
...
```
=== Request Processing Flow (Auth Middleware) ===
...
=== Response Processing Flow (Response Handler) ===
...
* **Module:** `src/auth/` and `src/handlers/`
...
- Must integrate with existing Lambda auth middleware and response handlers
...
9. CloudWatch dashboard "Aletheia-Usage" exists after running `provision.sh`
...
16. Logs Insights query `active-users.sql` returns unique user count for specified time period
...
**Rationale:** EMF provides the best balance of non-blocking performance and CloudWatch integration. Per-user analysis via Logs Insights avoids the cost explosion of high-cardinality custom metrics while still enabling abuse pattern detection.
...
2. `provision.sh` creates/updates CloudWatch dashboard via CLI
3. `provision.sh` creates SNS topic and CloudWatch alarm
...
jq . infrastructure/cloudwatch-dashboard.json

# Validate alarm JSON
jq . infrastructure/sns-alarm.json
```
...
| Dashboard JSON syntax error | Low | Med | jq validation in provision.sh before put-dashboard |
...
- [ ] provision.sh creates dashboard successfully
- [ ] SNS topic created
...
- `infrastructure/sns-alarm.json` ✓
- `infrastructure/contributor-insights-rules/top-talkers.json` ✓
- `infrastructure/logs-insights-queries/active-users.sql` ✓
