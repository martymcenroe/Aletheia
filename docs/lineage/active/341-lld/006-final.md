...
Path: C:\Users\mcwiz\Projects\Aletheia\docs\lld\active\LLD-341.md
Status: APPROVED
Reviews: 2
...
| `scripts/admin/adjust_cap.py` | Add | CLI tool for adjusting daily token cap (outside deployment artifact) |
...
moto = "^5.0.0"  # DynamoDB mocking for offline testing
```
...
| Admin Tool Location | `src/admin/`, `scripts/admin/`, Lambda | `scripts/admin/` | Keeps admin tools separate from deployment artifact; prevents accidental deployment to production |
...
- Admin tools must be excluded from Lambda deployment artifacts
...
| Log injection | Use structured logging (structlog/JSON) to ensure user_id is treated as a data field, not interpolated into log message strings | Addressed |
...
- Cache Secrets Manager retrieval at Lambda global scope
- TTL of 5 minutes for cached secret
- Reduces latency on warm starts and minimizes Secrets Manager API calls/costs
...
- [x] Data retention policy documented (30-day TTL on issuance records)
...
|----|---------|--------------|
| G1.1 | "Admin Tool Location: Move the admin tool to a separate directory outside the application source, such as `scripts/` or `tools/`." | YES - Changed from `src/admin/adjust_cap.py` to `scripts/admin/adjust_cap.py` in Section 2.1 and added rationale in Section 2.7 |
| G1.2 | "Unresolved Open Questions: Resolve and remove answered questions from Section 1" | YES - Removed "Should token cap be per-user or global?" from Open Questions as it's answered in Section 2.7 |
| G1.3 | "Missing Test Dependency: Add `moto` to the `dev-dependencies` in Section 2.2" | YES - Added moto to dev-dependencies in Section 2.2 |
| G1.4 | "Incomplete Security Spec: Define the sanitization strategy for Log Injection instead of leaving it as a TODO" | YES - Updated Section 7.1 to specify structured logging strategy |
| G1.5 | "Suggestion: Consider explicitly adding global-scope caching for the Secrets Manager retrieval" | YES - Added Secret Caching Strategy in Section 8.1 Performance |
| G1.6 | "Suggestion: Ensure the admin CLI tool has its own dependency definition if needed" | NOTED - Admin CLI shares project env; no separate deps needed |

### Review Summary

| Review | Date | Verdict | Key Issue |
|--------|------|---------|-----------|
| Gemini #1 | 2026-02-03 | REVISE | Admin tool location, missing moto dependency, incomplete log injection mitigation |
