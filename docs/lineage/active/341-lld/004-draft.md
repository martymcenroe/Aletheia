...
Updated By: Issue #341 LLD revision 2
Update Reason: Fixed test coverage mapping - added REQ-X markers to requirements and mapped tests to requirements
-->
...
2. **REQ-2:** Request with invalid JWT (bad signature) returns 401 Unauthorized
...
| ID | Scenario | Type | Requirements | Input | Expected Output | Pass Criteria |
|----|----------|------|--------------|-------|-----------------|---------------|
| 010 | JWT creation with valid inputs | Auto | REQ-5, REQ-6 | user_id="u123" | Valid JWT string | JWT decodes with user_id, exp (24h), iat, jti |
| 020 | JWT validation success | Auto | REQ-4 | Valid JWT | AuthResult(success=True, user_id="u123") | user_id extracted, analysis proceeds |
| 030 | JWT validation - expired | Auto | REQ-3 | Expired JWT | AuthResult(success=False, reason="token_expired") | 401 returned |
| 040 | JWT validation - bad signature | Auto | REQ-2 | Tampered JWT | AuthResult(success=False, reason="invalid_signature") | 401 returned |
| 050 | JWT validation - malformed | Auto | REQ-2 | "not.a.jwt" | AuthResult(success=False, reason="malformed") | 401 returned |
| 060 | Token cap - under limit | Auto | REQ-5 | count=5, cap=20 | (True, 6) | Request allowed, count incremented |
| 070 | Token cap - at limit | Auto | REQ-7 | count=20, cap=20 | (False, 20) | Request denied with 503 |
| 080 | Token cap - race condition | Auto | REQ-7 | Concurrent requests | One succeeds, one fails | Atomic increment verified |
| 090 | Auth middleware - no header | Auto | REQ-1, REQ-9 | {} | 401 + log | Log contains action: "auth_failed", reason |
| 100 | Auth middleware - wrong format | Auto | REQ-1, REQ-9 | "Basic xyz" | 401 + log | Log contains action: "auth_failed", reason |
| 110 | Auth middleware - valid | Auto | REQ-4 | "Bearer <valid>" | Handler called with user_id | Analysis proceeds |
| 120 | Admin CLI - set cap | Auto | REQ-8 | new_cap=30 | DynamoDB updated | Cap query returns 30 |
| 130 | Auth failure logging | Auto | REQ-9 | Any failure | Structured log | Contains action: "auth_failed", reason, timestamp |
| 140 | Secret retrieval | Auto | REQ-10 | N/A | Secret from Secrets Manager | Secret retrieved successfully |
| 150 | JWT dual secret validation | Auto | REQ-10 | JWT signed with old secret | AuthResult(success=True) | Falls back to secondary |
...
| Revision 2 | 2026-02-16 | PENDING | Added REQ-X markers and test coverage matrix |
