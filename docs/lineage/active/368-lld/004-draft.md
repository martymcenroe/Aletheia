...
Updated By: Issue #368 LLD revision
Update Reason: Fixed mechanical validation errors - added REQ-N suffixes to test scenarios and corrected Section 3 format
-->
...
|------|-------------|-------------|
| `src/auth/` | Add (Directory) | New directory for auth module components |
| `src/auth/handlers/` | Add (Directory) | New directory for auth handlers |
| `src/auth/handlers/metrics.py` | Add | New handler for `/metrics` endpoint with admin authentication |
| `src/auth/routes.py` | Add | Route mapping for `/metrics` → metrics handler |
| `src/lambda_auth_function.py` | Modify | Import and register new metrics route |
| `claude-staging/` | Existing | Static file directory (already exists) |
| `claude-staging/admin/` | Add (Directory) | New directory for admin dashboard files |
| `claude-staging/admin/metrics.html` | Add | Dashboard HTML page with Chart.js integration |
| `claude-staging/admin/metrics.js` | Add | Chart rendering, API fetching, auto-refresh logic |
| `claude-staging/admin/metrics.css` | Add | Responsive styling for dashboard |
| `claude-staging/admin/mock-metrics.json` | Add | Sample data fixture for offline development |
| `docs/architecture/api-reference.md` | Add | Document `/metrics` endpoint |
...
* **Module:** `src/auth/handlers/metrics.py`
...
| Static file location | New directory, existing claude-staging | claude-staging/admin/ | Reuses existing static hosting infrastructure |
...
|----|----------|------|-------|-----------------|---------------|
| 010 | No JWT provided (REQ-1) | Auto | GET /metrics without Authorization header | 401 Unauthorized | Status code 401, body contains "Unauthorized" |
| 020 | Invalid JWT signature (REQ-1) | Auto | GET /metrics with tampered JWT | 401 Unauthorized | Status code 401, body contains "Invalid token" |
| 030 | Non-admin tier access (REQ-2) | Auto | GET /metrics with JWT where tier='free' | 403 Forbidden | Status code 403, body contains "Admin access required" |
| 040 | Valid admin access (REQ-3) | Auto | GET /metrics with valid admin JWT | 200 OK with metrics JSON | Status 200, response contains all 7 metric keys |
| 050 | Dashboard prompts for auth (REQ-4) | Auto | Navigate to /admin/metrics without JWT | Authentication prompt displayed | Login form or redirect to login page |
| 060 | Dashboard displays 6 charts (REQ-5) | Auto | Navigate to /admin/metrics with admin JWT | 6 Chart.js charts rendered | document.querySelectorAll('canvas').length === 6 |
| 070 | 5xx error handling (REQ-6) | Auto | API returns 500 | Error message displayed | "Unable to load metrics. Retrying in 30 seconds..." visible |
| 080 | Auto-refresh updates timestamp (REQ-7) | Auto | Wait 5 minutes (mocked timer) | Timestamp updates | New timestamp displayed after refresh |
| 090 | Responsive at 375px (REQ-8) | Auto | Dashboard at 375px viewport width | No horizontal overflow | document.body.scrollWidth <= 375 |
| 100 | Performance warm response (REQ-9) | Auto-Live | Valid admin request with warm Lambda | Response within 1 second | Response time < 1000ms |
| 110 | No PII in response (REQ-10) | Auto | Full metrics response | No email, user_id, IP fields | JSON schema validation passes, PII fields absent |
| 120 | Mock mode loads fixture (REQ-11) | Auto | Dashboard with ?mock=true | Charts render from mock-metrics.json | Charts populated, no API call made |
| 130 | Cache hit behavior (REQ-12) | Auto | Two requests within 5 minutes | Second response has cached: true | cached field is true, DynamoDB not queried twice |
| 140 | Cache miss behavior (REQ-12) | Auto | Request after 5+ minutes | Fresh data fetched | cached field is false, new timestamp |
| 150 | Adoption data range (REQ-3) | Auto | Valid admin request | 90 data points in adoption array | adoption array length is 90 |
| 160 | Tier distribution accuracy (REQ-3) | Auto | DynamoDB with known tier counts | Matching tier counts | tiers.free + tiers.subscriber + tiers.admin matches total |
| 170 | Conversion rate calculation (REQ-3) | Auto | 10 converted of 100 eligible | rate: 10.0 | conversion.rate equals 10.0 |
| 180 | Revenue projection (REQ-3) | Auto | 50 subscribers at $9.99 | projected_monthly: 499.50 | revenue.projected_monthly equals 499.50 |
| 190 | CORS headers (REQ-3) | Auto | Valid admin request | CORS header set to aletheia.study | Access-Control-Allow-Origin: https://aletheia.study |
| 200 | Geographic data no IP (REQ-10) | Auto | Geography metrics | Country codes only | Keys are 2-letter country codes, no IP addresses |
...
poetry run playwright test tests/e2e/metrics_dashboard.spec.ts
```
...
- [ ] docs/architecture/api-reference.md created with /metrics endpoint
- [ ] Files added to 0003-file-inventory.md

### Review
- [ ] Code review completed
- [ ] 0809 Security Audit - PASS (admin authentication, no PII exposure)
- [ ] 0810 Privacy Audit - PASS (aggregate-only data)
...
|--------|------|---------|-----------|
| Mechanical Validation #1 | 2026-02-16 | REJECTED | Invalid file paths - directories don't exist |
| Revision #1 | 2026-02-16 | REJECTED | 25% coverage - missing REQ-N suffixes in test scenarios |
| Revision #2 | 2026-02-16 | PENDING | Fixed REQ-N suffixes, expanded test coverage to all 12 requirements |
