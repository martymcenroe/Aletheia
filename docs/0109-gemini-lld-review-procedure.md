# Gemini LLD Review Prompt

Use this prompt when assigning Gemini to review an LLD before implementation begins.
...
| Category | Key Questions |
|----------|---------------|
| **Testing** | All scenarios covered? Willison Protocol (tests fail on revert)? Mocks for external deps? Data hygiene (no real slurs in tests)? |
| **Data Pipeline** | Where does data come from? How does it get there? Test fixtures defined? Deployment pipeline documented? Separate utility needed? |
