...
| 2025-12-09 | Hate Filter Auto-Update | The local `denylist.json` (Denylist) is static and requires manual updates. | Automated harvesting via `harvest_rsdb.py` is viable. | **Action:** Determine trigger: Reactive (EventBridge on Semantic failure) vs. Scheduled (Cron). Ref #1012. |
