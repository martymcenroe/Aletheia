...
# Check if there's already a recent entry (within last 10 minutes)
...
if [ -f "$log_file" ]; then
    ...
    if [ -n "$last_entry" ]; then
        ...
        # Simple check: if last entry was in the same hour, skip
        last_hour="${last_entry:0:2}"
        current_hour="${current_time:0:2}"
        if [ "$last_hour" = "$current_hour" ]; then
            echo "[Stop Hook] Recent session log entry exists, skipping" >&2
            exit 0
        fi
    fi
fi
